...
function main()
    ...
    fix_thetas            = false
    kl_weight             = Float32(0.0) # beta-VAE weight on KL(N(mu,sigma²) || N(0,I))
    latent_lr_multiplier  = Float32(1)         # LR multiplier for latent variational parameters
    ...
    elseif model_arch == :conv
        ...
        conv_channels     = [32, 16, 8, 4]   # output channels per block (length = n_blocks)
    ...
    if resume_phase1
        ...
        ps, st = load_checkpoint(model_dir)
    ...
    if model_arch == :fourier
        ...
        psi_hat, _ = NN._decode_psi_hat(model, x_plot, ps_cpu, st_cpu)
        gen_omega_hat = .-model.grid.lap .* psi_hat   # |k|² · ψ̂
