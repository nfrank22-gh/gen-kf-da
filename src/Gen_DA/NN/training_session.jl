...
function TrainingSession(
    ...
    if training_mode == :observations
        ...
        prepare_epoch_fn = function(full_batches)
            ...
            return Reactant.to_rarray(u_cpu), Reactant.to_rarray(v_cpu)
        end
        get_data_batch_fn = function(data_all, cols)
            u_all, v_all = data_all
            return (u_all[:, cols], v_all[:, cols])
        end
        ...
        prepare_epoch_fn = function(full_batches)
            ...
            return (Reactant.to_rarray(omega_cpu),)
        end
        get_data_batch_fn = function(data_all, cols)
            omega_all, = data_all
            return (omega_all[:, :, cols],)
        end
        train_loss_fn = function(m, params, states, data)
    ...
    x_eval_dummy  = Reactant.to_rarray(zeros(T, latent_dim, n_eval))
    compiled_eval = Reactant.@compile eval_decoder_vort(
        model, N, x_eval_dummy, tstate.parameters, tstate.states)
    ...
    return TrainingSession(
        ...
        sampler, compiled_eval, eval_omega,
        Float32[], Float32[], Int[],
    )
end
...
function train!(session::TrainingSession)
    ...
    for epoch in 1:session.n_epochs
        ...
        x_all      = Reactant.to_rarray(x_cpu)
        thetas_all = Reactant.to_rarray(thetas_cpu)

        batch_losses = Vector{Any}(undef, session.n_full)
        for i in 1:session.n_full
            x, thetas, cols = get_batch(session.sampler, x_all, thetas_all, i)
        ...
        avg_loss   = sum(convert(T, l) for l in batch_losses) / session.n_full
