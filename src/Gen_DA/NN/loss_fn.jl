...
function sinkhorn_divergence(P::AbstractMatrix, Q::AbstractMatrix, eps::Real; n_iter::Int=100)
    C_PQ            = _sq_cost(P, Q)
    scale           = mean(C_PQ)
    ot_PQ, residual = _sinkhorn_ot(C_PQ ./ scale,          eps, n_iter)
    ot_PP, _        = _sinkhorn_ot(_sq_cost(P, P) ./ scale, eps, n_iter)
    ot_QQ, _        = _sinkhorn_ot(_sq_cost(Q, Q) ./ scale, eps, n_iter)
...
function loss_fn(model, NDOF, ps, st,
  ...
  if recon_loss == :sinkhorn
      rec, sinkhorn_res = sinkhorn_divergence(P, Q, sinkhorn_eps; n_iter=sinkhorn_n_iter)
      return rec + _total_kl(mu, log_sigma, kl_weight), st, (; sinkhorn_residual=sinkhorn_res)
  end
